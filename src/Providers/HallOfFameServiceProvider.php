...
use Whozidis\HallOfFame\Http\Middleware\AfterPasswordChangeActions;
use Whozidis\HallOfFame\Http\Middleware\StrongPasswordMiddleware;
...
class HallOfFameServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        HallOfFameTheme::registerThemeViews();
        ...
        $this->app->booted(function () {
            ...
            try {
                ...
                if ($route = $routes->getByName('customer.post.change-password')) {
                    $route->middleware([
                        StrongPasswordMiddleware::class,
                        AfterPasswordChangeActions::class,
                    ]);
                }

                // Enforce strong password on registration as well
                if ($route = $routes->getByName('customer.register.post')) {
                    $route->middleware(StrongPasswordMiddleware::class);
                }
            } catch (\Throwable $e) {
